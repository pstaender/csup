...
googleapis.discover("drive", "v2").execute(function(err, client) {
  ...
  return process.stdin.pipe(request.post({
    ...
    if (err) {
      return console.error("Error during upload: ", (err != null ? err.message : void 0) || err || (res != null ? (_ref = res.body) != null ? _ref.error : void 0 : void 0));
    } else {
      if (body != null ? body.error : void 0) {
        return console.error(body != null ? body.error : void 0);
      } else {
        fileId = body.id;
        ...
        return req.withAuthClient(auth).execute(function(err, res) {
          if (err) {
            console.log("* " + fileId);
            return console.error('Could not rename uploaded file');
          } else {
            return console.log("* " + fileId + " -> " + res.title);
          }
        });
      }
    }
